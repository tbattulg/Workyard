...
- R2 file storage: deferred from the current build

Do not reuse or modify the existing Invoice Maker Pages project or D1 database.

## Required setup

1. Add Clerk, Clerk webhook signing, Resend, Mapbox, Sentry, and Turnstile values as encrypted secrets.
2. Add `VITE_CLERK_PUBLISHABLE_KEY`, `CLERK_PUBLISHABLE_KEY`, `VITE_MAPBOX_PUBLIC_TOKEN`, and `SUPPORT_EMAIL` as plain deployment variables when real values are available.
3. Apply migrations to preview, verify the preview deployment, then apply to production.
...
2. Add the encrypted provider secrets:
   - `CLERK_SECRET_KEY`
   - `CLERK_JWT_KEY`
   - `RESEND_API_KEY`
   - `MAPBOX_SECRET_TOKEN`
...
- File uploads remain unavailable while R2 is deferred.
